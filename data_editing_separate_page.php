 ...
 */
    ...
    class v_data_editing_separate_pagePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Data Input Forms.Separate Page');
            $this->SetMenuLabel('Separate Page');
    
            $this->dataset = new TableDataset(
                MySqlIConnectionFactory::getInstance(),
                GetConnectionOptions(),
                '`v_data_editing_separate_page`');
            ...
            $this->dataset->AddCustomCondition(EnvVariablesUtils::EvaluateVariableTemplate($this->GetColumnVariableContainer(), 'v_data_editing_separate_page.Population > 8000000'));
        }
    
        protected function DoPrepare() {
            $this->setDescription(file_get_contents("external_data/doc/data_editing_separate_page.html"));
        }
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            ...
            $main_editor->SetHandlerName('filter_builder_v_data_editing_separate_page_countrycode_search');
            
            $multi_value_select_editor = new RemoteMultiValueSelect('countrycode', $this->CreateLinkBuilder());
            $multi_value_select_editor->SetHandlerName('filter_builder_v_data_editing_separate_page_countrycode_search');
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'countrycode', 
                $editor, 
                $this->dataset, 'Code', 'Name', $lookupDataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        protected function doRegisterHandlers() {
            ...
            $lookupDataset->setOrderByField('Name', 'ASC');
            $handler = new DynamicSearchHandler($lookupDataset, 'filter_builder_v_data_editing_separate_page_countrycode_search', 'Code', 'Name', null, 20);
    ...
    try
    {
        $Page = new v_data_editing_separate_pagePage("v_data_editing_separate_page", "data_editing_separate_page.php", GetCurrentUserPermissionsForPage("v_data_editing_separate_page"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("v_data_editing_separate_page"));
