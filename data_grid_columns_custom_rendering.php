 ...
 */
    ...
    class v_data_grid_columns_custom_renderPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Grid Columns.Custom Rendering');
            $this->SetMenuLabel('Custom Rendering');
    
            $this->dataset = new TableDataset(
                MySqlIConnectionFactory::getInstance(),
                GetConnectionOptions(),
                '`v_data_grid_columns_custom_render`');
        ...
        protected function DoPrepare() {
            $this->setDescription(file_get_contents("external_data/doc/data_grid_columns_custom_rendering.html"));
            $this->setDetailedDescription(extractMethodCode($this, 'doCustomRenderColumn'));
        }
        ...
        { 
            if ($fieldName === 'population') {
               $customText = sprintf(
                   '~%s M<br><small style="opacity: .6">%s</small>',
                   number_format($fieldData / 1000000, 3),
                   number_format($fieldData, 0)
               );
               $handled = true;
            }
        }
    ...
    try
    {
        $Page = new v_data_grid_columns_custom_renderPage("v_data_grid_columns_custom_render", "data_grid_columns_custom_rendering.php", GetCurrentUserPermissionsForPage("v_data_grid_columns_custom_render"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("v_data_grid_columns_custom_render"));
