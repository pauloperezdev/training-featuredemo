 ...
 */
    ...
    class v_data_grid_custom_grid_toolbar_templatePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Data Grid.Custom Grid Toolbar Template');
            $this->SetMenuLabel('Custom Toolbar Template');
    
            $this->dataset = new TableDataset(
                MySqlIConnectionFactory::getInstance(),
                GetConnectionOptions(),
                '`v_data_grid_custom_grid_toolbar_template`');
        ...
        protected function DoPrepare() {
            $this->setDescription(file_get_contents("external_data/doc/data_grid_custom_grid_toolbar_template.html"));
            
            $this->setDetailedDescription(
              extractMethodCode($this, 'doGetCustomTemplate') .
              '<div id="custom-grid-toolbar-template" style="display: none;">' .
              extractTemplateFileCode('components/templates/custom_templates/custom_grid_toolbar.tpl') .
              '</div>'
            );
        }
        ...
        protected function CreateGrid()
        {
            ...
            $defaultSortedColumns[] = new SortColumn('rating', 'ASC');
            ...
            $this->setAllowedActions(array('view', 'insert'));
        ...
        {
            if (GetApplication()->GetGETValue('random_movie') == 'on') {
              $sql = sprintf('SELECT id FROM (%s) SOURCE ORDER BY RAND() LIMIT 1', $this->dataset->GetSelectCommand()->GetSQL(false));
              $randomMovieId = $this->GetConnection()->ExecScalarSQL($sql);
              if ($randomMovieId) {
                $this->dataset->AddCustomCondition("id = $randomMovieId");                      
              }        
            }
        }
    ...
    try
    {
        $Page = new v_data_grid_custom_grid_toolbar_templatePage("v_data_grid_custom_grid_toolbar_template", "data_grid_custom_grid_toolbar_template.php", GetCurrentUserPermissionsForPage("v_data_grid_custom_grid_toolbar_template"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("v_data_grid_custom_grid_toolbar_template"));
