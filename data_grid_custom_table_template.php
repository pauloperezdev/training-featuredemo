 ...
 */
    ...
    class v_data_grid_custom_table_templatePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Data Grid.Custom Table Template');
            $this->SetMenuLabel('Custom Table Template');
    
            $this->dataset = new TableDataset(
                MySqlIConnectionFactory::getInstance(),
                GetConnectionOptions(),
                '`v_data_grid_custom_table_template`');
        ...
        protected function DoPrepare() {
            $this->setDetailedDescription(extractMethodCode($this, 'doGetCustomTemplate'));
            $this->setDescription(file_get_contents("external_data/doc/data_grid_custom_table_template.html"));
        }
        ...
        protected function getFiltersColumns()
        {
            return array(
                ...
                new FilterColumn($this->dataset, 'release_date', 'release_date', 'Released'),
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(120);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            //
            // View column for release_date field
            //
            $column = new DateTimeViewColumn('release_date', 'release_date', 'Released', $this->dataset);
            $column->SetOrderable(true);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new CustomEditColumn('Released', 'release_date', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        {
            if ($part == PagePart::Grid && $mode == PageMode::ViewAll && $this->getGrid()->getViewMode() == ViewMode::TABLE) {
                $result = 'custom_table.tpl';
            }
        }
    ...
    try
    {
        $Page = new v_data_grid_custom_table_templatePage("v_data_grid_custom_table_template", "data_grid_custom_table_template.php", GetCurrentUserPermissionsForPage("v_data_grid_custom_table_template"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("v_data_grid_custom_table_template"));
