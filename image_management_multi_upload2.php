 ...
 */
    ...
    class album02_photoPage extends DetailPage
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Photo');
            $this->SetMenuLabel('Photo');
            ...
            $this->dataset->AddLookupField('album_id', 'album', new IntegerField('id'), new StringField('title', false, false, false, false, 'album_id_title', 'album_id_title_album'), 'album_id_title_album');
        }
        ...
        protected function getFiltersColumns()
        {
            return array(
                ...
                new FilterColumn($this->dataset, 'added', 'added', 'Added'),
                new FilterColumn($this->dataset, 'album_id', 'album_id_title', 'Album Id')
            );
        }
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            ...
            $main_editor = new DateTimeEdit('added_edit', false, 'Y-m-d');
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            ...
            $column->setSourcePrefixTemplate('external_data/images/album/album_photo/');
            $column->setImageHintTemplate('%title%');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            //
            // View column for added field
            //
            $column = new DateTimeViewColumn('added', 'added', 'Added', $this->dataset);
            $column->SetOrderable(true);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new UploadFileToFolderColumn('Image', 'image', $editor, $this->dataset, false, false, 'external_data/images/album/album_photo', '%original_file_name%', $this->OnFileUpload, true);
            ...
            //
            // Edit column for added field
            //
            $editor = new DateTimeEdit('added_edit', false, 'Y-m-d');
            $editColumn = new CustomEditColumn('Added', 'added', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        }
        ...
        private function AddMultiUploadColumn(Grid $grid)
        {
            ...
            $editColumn = new UploadFileToFolderColumn('Images', 'image', $editor, $this->dataset, false, false, 'external_data/images/album/album_photo', '%original_file_name%', $this->OnFileUpload, true);
            $editColumn->SetReplaceUploadedFileIfExist(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
        ...
        protected function CreateGrid()
        {
            ...
            $this->setAllowedActions(array('view', 'insert', 'edit', 'multi-upload', 'delete', 'multi-delete'));
    ...
    class album02Page extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->SetTitle('Image Management.Multi Upload - 2');
            $this->SetMenuLabel('Multi Upload - 2');
        ...
        protected function DoPrepare() {
            $this->setDescription(file_get_contents("external_data/doc/image_management_multi_upload2.html"));
        }
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserPermissionsForPage('album02.photo')->HasViewGrant() && $withDetails)
            {
            //
            // View column for album02_photo detail
            //
            $column = new DetailColumn(array('id'), 'album02.photo', 'album02_photo_handler', $this->dataset, 'Photo');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            }
        ...
        protected function doRegisterHandlers() {
            $detailPage = new album02_photoPage('album02_photo', $this, array('album_id'), array('id'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserPermissionsForPage('album02.photo'), 'UTF-8');
            $detailPage->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource('album02.photo'));
            $detailPage->SetHttpHandlerName('album02_photo_handler');
            $handler = new PageHTTPHandler('album02_photo_handler', $detailPage);
    ...
    try
    {
        $Page = new album02Page("album02", "image_management_multi_upload2.php", GetCurrentUserPermissionsForPage("album02"), 'UTF-8');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("album02"));
